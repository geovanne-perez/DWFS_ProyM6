	...
	try {
		...
		const shoppingCart = await CartModel.findById(id);
         ...
         if (shoppingCart == null) {
            ...
            return res.status(409).send({ message: `Carrito de compras ${id} no encontrado` });
        }
...
const AddToCart = (async (req=request,res=response) => {
    try {
        ...
        const shoppingCart = new CartModel({userId,carId,active:true,closed:false});
		...
		// Encontrar auto existente
         ...
         if (auto == null) {
             ...
             return res.status(409).send({ message: `Vehículo con ID ${carId} no encontrado` });
         }
         
		// Enviar mensaje si el auto no tiene status 1 (Disponible)
		if (auto.status != 1) {
			console.log(`Vehículo con ID ${carId} no disponible`);
			return res.status(409).send({ message: `Vehículo con ID ${carId} no disponible para apartado. Verifique nuevamente.` });
		}
         // Actualizar auto		
         auto.status = 2;	
		...
		const updatedAuto = await CarModel.findByIdAndUpdate(carId,auto,{new:true});

        console.log("Estatus de Auto ", updatedAuto.id, "actualizado a ", updatedAuto.status);
		res.status(200).json({
...
const CheckoutCart = (async (req=request,res=response) => {
    try {
        ...
        const shoppingCart = await CartModel.findById(id);
		console.log("Carrito de compras encontrado:", shoppingCart.id," del usuario: ", shoppingCart.userId);
		...
		if (shoppingCart.active == false || shoppingCart.closed == true) {
		   ...
		   return res.status(409).send({ message: `Carrito de compras ${id} no disponible, intente nuevamente` });
	   }
		...
		const updatedCart = await CartModel.findByIdAndUpdate(id,shoppingCart,{new:true});
		const updatedAuto = await CarModel.findByIdAndUpdate(carId,auto,{new:true});
